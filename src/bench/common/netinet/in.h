...
# define htons(x)	(x)
 ...
 static __inline__ uint16_t
  spe_flip (uint16_t __arg)
  {
    register uint16_t __result;
    __result = (__arg << 8) & 0xff00;
    __result |= (__arg >> 8) & 0x00ff;
    return __result;
  }
 static __inline__ uint32_t
  spe_lflip (uint32_t __arg)
  {
...
#if 0  /*marvin */
  /* For other little-endian machines, using assembly code.  */
  /* Convert a host long to a network long.  */
  static __inline__ uint32_t
  htonl (uint32_t __arg)
  {
    register uint32_t __result;
    __result = (__arg >> 24) & 0x000000ff;
    __result |= (__arg >> 8) & 0x0000ff00;
...
static __inline__ uint32_t
ntohl (uint32_t __arg)
{
  return htonl (__arg);
}
/* Convert a network short to a host short.  */
static __inline__ uint16_t
ntohs (uint16_t __arg)
{
  return htons (__arg);
}
